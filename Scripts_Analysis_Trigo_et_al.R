...
###### Analysis for the 2.2. Experimental design section ######

adultos <- data.frame(
  adentro = c(38, 32.5, 37, 24.5, 23.5),
  afuera = c(41.5, 36.5, 44.5, 33.5, 24.5)
)

wilcox.test(adultos$adentro, adultos$afuera, paired = TRUE)


blanco <- data.frame(
  adentro = c(14.5, 15.5, 7, 11.5, 5),
  afuera = c(11, 10.5, 7, 5, 4.5)
)

wilcox.test(blanco$adentro, blanco$afuera, paired = TRUE)

colorado <- data.frame(
  adentro = c(5.5, 1.5, 6, 1.5, 1),
  afuera = c(4) # revisar esto
)

wilcox.test(colorado$adentro, colorado$afuera, paired = TRUE)

densidad_aparente <- data.frame(
  adentro = c(1.242518, 1.194572, 1.074969, 1.182632, 1.070574),
  afuera = c(1.1444, 1.25135, 1.204448, 1.225248, 1.067916)
)

wilcox.test(densidad_aparente$adentro, densidad_aparente$afuera, paired = TRUE)
